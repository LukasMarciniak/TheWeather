...
class CurrentWeather {
    ...
    var date: String{
        ...
        dateForrmater.dateStyle = .full
        dateForrmater.timeStyle = .none
        
        let currentDate = dateForrmater.string(from: Date())
        _date = currentDate
        return _date
    }
            ...
            if let dict = result.value as? Dictionary<String, AnyObject> {
                ...
                if let main = dict["main"] as? Dictionary<String, AnyObject> {
                    if let currentTemparature = main["temp"] as? Double {
                        let kelvToCels = (currentTemparature - 273.15)
                        self._currentTemp = round(kelvToCels*10)/10
